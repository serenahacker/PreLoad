...
def Noise(dataset, train=True, batch_size=None, size=None):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
    ...
    if size == None:
        ...
        dataset_val, _ = data_utils.random_split(dataset, (size, len(dataset)-size), generator=torch.Generator().manual_seed(42))
...
def UniformNoise(dataset, delta=1, train=False, size=2000, batch_size=None, kind='type1'):
    if batch_size == None:
        if train:
            batch_size=train_batch_size
        else:
    ...
    if kind == 'type1':
        ...
        v = torch.randn((size, shape[0]*shape[1]*shape[2])) 
        v = v / torch.norm(v, dim=1, keepdim=True)
        v = v.reshape(((size,) + shape))
        data = torch.rand((size,) + shape) + delta*v
...
def FarAway(dataset, train=False, size=2000, batch_size=None):
    return UniformNoise(dataset, 10000, train, size, batch_size)

def FarAway2(dataset, train=False, size=2000, batch_size=None):
    return UniformNoise(dataset, 10000, train, size, batch_size, kind='type2')
...
def Tiny300k(dataset='CIFAR10', batch_size=None, meanstd=None):
    assert dataset in ['MNIST', 'FMNIST', 'CIFAR10', 'SVHN', 'CIFAR100'], 'Invalid dataset.'
    ...
    raw_imgs = np.load(f'{path}/300K_random_images.npy')  # (300k, 32, 32, 3)
    raw_imgs = torch.tensor(raw_imgs).permute(0, 3, 1, 2)  # (n, c, h, w)
    dummy_labels = torch.ones(raw_imgs.shape[0])
    dataset = Tiny300kDataset(raw_imgs, dummy_labels, transform=transform)
...
class Tiny300kDataset(data_utils.Dataset):
    """
    TensorDataset with support of transforms.

    Parameters:
    -----------
    tensors: sequence of torch.Tensors
        The transform will only be applied to the first tensor.
...
datasets_dict = {
    ...
    'FarAway2': FarAway2,
    'Noise': Noise,
    'CorruptedCIFAR10': CorruptedCIFAR10,
 }
